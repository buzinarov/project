#!/usr/bin/python2.4
#
# Small script to show MongoDB and pymongo together
#
...
class PropertyDAO:
    ...
    def __init__(self):
        # self.postgreSQL_pool = SimpleConnectionPool(5, 20, user = "postgres",
        #                                      password = "990274",
        #                                      host = "LocalHost",
        #                                      port = "5432",
        #                                      database = "postgres")
    ...
    def getRecords(self, limit):
        ...
        print("Select from postgres database")
        if (limit > 0):
            query = "SELECT id, fulldescription, characteristics, title from rentproperty where (fulldescription is not null OR fulldescription <> '') limit " + str(
                limit)
        else:
            query = "SELECT id, fulldescription, characteristics, title from rentproperty where (fulldescription is not null OR fulldescription <> '')"
    ...
    def getNoSuitLaundryRecords(self, limit):
        ...
        if (limit > 0):
            query = "SELECT id, fulldescription, characteristics, title from rentproperty where suit_laundry = false limit " + str(
                limit)
        else:
    ...
    def getNoRoomSizeRecords(self, limit):
        ...
        if (limit > 0):
            query = "SELECT id, fulldescription, characteristics, title from rentproperty where bedrooms <= 0 limit " + str(
                limit)
        else:
    ...
    def getNoSizeRecords(self, limit):
        ...
        if (limit > 0):
            query = "SELECT id, fulldescription, title, characteristics from rentproperty where size_sqft <= 0 limit " + str(
                limit)
        else:
    ...
    def getNoBathRecords(self, limit):
        ...
        if (limit > 0):
            query = "SELECT id, fulldescription, title, characteristics from rentproperty where bath is null limit " + str(
                limit)
        else:
    ...
    def getRecordsWithNoLocation(self, limit=0):
        ...
        if (limit > 0):
            query = "SELECT id, fulldescription, title, characteristics, link from rentproperty where location is null limit " + str(
                limit)
        else:
    ...
    def updateRecord(self, property: Property):
        ...
        cur = conn.cursor()
        cur.execute(query, (
        property.rooms, property.size, property.profMan, property.nopet, property.suitLaundry, property.parkStall,
        property.availNow, property.amenities, property.nearSchool, property.brandNew, property.id))
        conn.commit()
