...
from scraper.text_mining import houseservice
  ...
  #                  property.setUpdate()
   ...
   #     return property;
    ...
    def populateTokens(self, property: Property, sentences):
        searchfor = ['professionally managed', 'no pets', 'parking stall', 'available now', 'building amenities',
                     'near school', 'brand new', 'suite laundry']
    ...
    def getLocationFromSentences(self, sentences, link):
        ...
        locations = ['burnaby', 'new westminster', 'richmond', 'stanley park', 'surrey', 'vancouver', 'downtown',
                     'abbotsford', 'granville', 'brentwood', 'delta', 'langley', 'white-rock', 'coquitlam',
                     'new-westminster', 'fleetwood', 'port-moody']
    ...
    def tryGetBedroomFromDescription(self, property: Property):
        ...
        for i in range(0, len(words)):
            ...
            if words[i] in ['bedroom', 'bedrooms', 'bed', 'br', 'brm', 'bdrm', 'bdr'] or 'bed' in words[i] or 'bd' in \
                    words[i]:
    ...
    def tryGetSizeFromDescription(self, property: Property):
        ...
        desc = str(property.characteristics).lower() + ' ' + str(property.title).lower() + ' ' + str(
            property.description).lower()  # parse the texto to lowercase
        words = desc.split(' ');
        for i in range(0, len(words)):

            # check for a split pattern in text and apply algorithm to identify the number
            if words[i] in ['square', 'sqft', 'sq.', 'sqt', 'sqf', 'sqft)', 'sq.ft.', 'sqft.', 'sq', 'sqft,', 'sf',
                            'sq.ft', 'sq.ft.,', 'sqft).', 'sq/ft', 'sq.ft']:
